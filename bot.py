...
def get_weather_factor(stadium_city):
    ...
    try:
        ...
        factor=1.0
        if temp<5: factor*=0.95
        elif temp>30: factor*=0.9
        if rain>0: factor*=0.92
        if snow>0: factor*=0.9
        return factor
    except:
        return 1.0
...
# ==============================
# Запуск с учетом активного event loop (Railway)
# ==============================
